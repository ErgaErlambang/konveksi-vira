    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">Testimoni Management</h3>
            </div>
            <div class="card-toolbar">
                <div class="dropdown dropdown-inline mr-2">
                    <a href="{{ route('admin.testimoni.create') }}" class="btn btn-primary font-weight-bolder"><i class="la la-plus"></i>New Record</a>
                </div>
            </div>
        </div>
        <div class="card-body">
                ...
                <tbody>
                    @foreach ($testimonis as $testimoni)
                        <tr>
                            <td>{{ $testimoni->id }}</td>
                            <td>
                                <img src="{{ asset('uploads/testi/'.$testimoni->image) }}" alt="testimoni" class="img-fluid" width="150">
                            </td>
                            <td>{{ $testimoni->created_at }}</td>
                            <td class="text-center mx-auto">
                                <div class="row p-2">
                                    <a href="{{ route('admin.testimoni.edit', $testimoni->id) }}" class="btn btn-link btn-sm btn-info btn-just-icon like mr-1" style="display: block; margin-block-end: 1.5em;">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.testimoni.destroy', $testimoni->id) }}" class="form-delete" method="POST">
                                        @csrf
